...
def match_data(path,poslist,neglist):
    ...
    for i in range(len(all_path)):
        patient_id.append(all_path[i].split("/")[7][0:9])
...
def split_train_val_test(path,ratio,idlist):
    ...
    np.save('/work/07034/byz/maverick2/GNN/CPTACtrain.npy', np.array(train_ID))
    np.save('/work/07034/byz/maverick2/GNN/CPTACval.npy', np.array(val_ID))
    np.save('/work/07034/byz/maverick2/GNN/CPTACtest.npy', np.array(test_ID))
    np.save('/work/07034/byz/maverick2/GNN/CPTACtrain_path_t.npy', np.array(train_path))
    np.save('/work/07034/byz/maverick2/GNN/CPTACval_path_t.npy', np.array(val_path))
    np.save('/work/07034/byz/maverick2/GNN/CPTACtest_path_t.npy', np.array(test_path))
    return train_path,val_path,test_path

#train,val,test=split_train_val_test(all,[0.7,0.3,0],allid)
...
x='/work/07034/byz/maverick2/GNN/CPTACtrain_path_dn_t.npy'
y='/work/07034/byz/maverick2/GNN/CPTACval_path_t.npy'
z='/work/07034/byz/maverick2/GNN/CPTACtest_path_t.npy'
...
#np.save('/work/07034/byz/maverick2/GNN/CPTACtrain_path_dn_t.npy', np.array(train))
#np.save('/work/07034/byz/maverick2/GNN/CPTACval_path_dn_t.npy', np.array(val))
#np.save('/work/07034/byz/maverick2/GNN/test_path_dn_m_t_100.npy', np.array(test))

def read_label(path,poslist,neglist):
    label_path=path
    tmplabel=label_path.split("/")[-1][0:9]
...
def pat_AUC(model1, data1, NEG1, POS1, type):
    ...
    np.save('/work/07034/byz/maverick2/GNN/CPTACtruelabel_' + type + '.npy', np.array(truelabel))
    np.save('/work/07034/byz/maverick2/GNN/CPTACpredlabel_' + type + '.npy', np.array(predlabel))
    np.save('/work/07034/byz/maverick2/GNN/CPTACpat_' + type + '.npy', np.array(ID))
    fpr, tpr, thresholds = roc_curve(truelabel, predlabel)
    np.save('/work/07034/byz/maverick2/GNN/CPTACtpr_' + type + '.npy', np.array(tpr))
    np.save('/work/07034/byz/maverick2/GNN/CPTACfpr_' + type + '.npy', np.array(fpr))
    roc_auc = auc(fpr, tpr)
    print(type, "CPTAC_AUC:", roc_auc)
...
len_train_loss=0
if args.c==1:
    checkpoint = torch.load(args.mpath)
