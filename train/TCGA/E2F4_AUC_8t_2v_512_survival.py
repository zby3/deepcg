...
all3=glob(all3)
all=all1+all2+all3
myinf1="/work/07034/byz/maverick2/GNN/Thorsson_2018_TCGA_immunelandscape.csv"
info=pd.read_csv(myinf1,sep=",")
info=info[info['TCGA Study']=="LUAD"]
info=info[info['OS Time'].notna()]
sam1=np.where(info.loc[:,"OS Time"]>730)
sam2=np.where(info.loc[:,"OS Time"]<=730)
info1=info.iloc[sam1]
info2=info.iloc[sam2]
POS=info1['TCGA Participant Barcode'].tolist()
NEG=info2['TCGA Participant Barcode'].tolist()
allid=POS+NEG
...
train.extend(val)
val=test
train=[tmp for tmp in train if tmp.rsplit('/',1)[1][0:12] in allid]
val=[tmp for tmp in val if tmp.rsplit('/',1)[1][0:12] in allid]
...
np.save('/work/07034/byz/maverick2/GNN/train_path_dn_t_surv.npy', np.array(train))
np.save('/work/07034/byz/maverick2/GNN/val_path_dn_t_surv.npy', np.array(val))
